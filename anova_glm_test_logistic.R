...
beta_0 <- -0.5
beta_1 <- 0
beta_2_seq <- seq(0,0.8,by=0.1)/50
beta_3 <- 0
...
for (beta_2 in beta_2_seq){
  ...
  for (i in c(1:n_sim)){
    cat(i,"\n")
    y_sim <- rbinom(n = nrow(mri), size=1, plogis(beta_0 + mri$weight*beta_1 +
                     mri$height*beta_2+mri$alcoh*beta_3))
    curr_reg <- regress("odds", y_sim~weight+height+alcoh,data=mri)
    null_reg <- regress("odds", y_sim~weight,data=mri)
  ...
  for (i in c(1:n_sim)){
    ...
    cat(i,"\n")
    y_sim <- rbinom(n = nrow(mri_sub),size=1 ,plogis(beta_0 + mri_sub$weight*beta_1 +
                     mri_sub$height*beta_2+mri_sub$alcoh*beta_3))
    curr_reg <- regress("odds", y_sim~weight+height+alcoh,data=mri_sub)
    null_reg <- regress("odds", y_sim~weight,data=mri_sub)
    curr_p_wald <- anova(null_reg, curr_reg,test = "Wald",robustSE = TRUE)$printMat[,4]
    curr_p_naive <- anova(null_reg, curr_reg,test = "Wald",robustSE = FALSE,
                          useFdstn = FALSE)$printMat[,3]
    curr_p_LRT <- anova(null_reg, curr_reg,test = "LRT")$printMat[,3]
...
save(df_p0_plot_data,
     file="~/Desktop/summer_2022/power_anova_glm_binomial.RData")
