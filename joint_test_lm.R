...
p_val_collection_735_wald_no_f <- rep(NA, n_sim)
p_val_collection_100_wald <- rep(NA, n_sim)
p_val_collection_100_naive <- rep(NA, n_sim)
p_val_collection_100_wald_no_f <- rep(NA, n_sim)
p_val_collection_30_wald <- rep(NA, n_sim)
p_val_collection_30_naive <- rep(NA, n_sim)
p_val_collection_30_wald_no_f <- rep(NA, n_sim)
p_val_collection_10_wald <- rep(NA, n_sim)
p_val_collection_10_naive <- rep(NA, n_sim)
p_val_collection_10_wald_no_f <- rep(NA, n_sim)

df_p0_summary_wald <- data.frame()
df_p0_summary_naive<- data.frame()
df_p0_summary_wald_no_f <- data.frame()

for (beta_2 in beta_2_seq){
  # full 735 samples
  for (i in c(1:n_sim)){
    cat(i,"\n")
    y_sim <- beta_0 + mri$weight*beta_1 + mri$height*beta_2 + rnorm(nrow(mri), 
                     sd = rgamma(nrow(mri),shape=10))
    curr_reg <- regress("mean", y_sim~weight+height,data=mri)
    ...
    curr_p <- curr_joint_naive$printMat[,3]
    ...
    p_val_collection_735_wald_no_f[i] <- curr_p_wald_no_f
  }
  ...
  for (i in c(1:n_sim)){
    ...
    p_val_collection_100_wald_no_f[i] <- curr_p_wald_no_f
  }
  ...
  df_p0_summary_wald_no_f <- rbind(df_p0_summary_wald_no_f, 
                             data.frame(
                               p_val_collection_10 = p_val_collection_10_wald_no_f,
                               p_val_collection_30 = p_val_collection_30_wald_no_f,
                               p_val_collection_100 = p_val_collection_100_wald_no_f,
                               p_val_collection_735 = p_val_collection_735_wald_no_f,
                               beta_2=beta_2,
                               type="LRT"))
}
...
df_p0_plot_data <- rbind(df_p0_summary_wald, 
                         df_p0_summary_naive,
                         df_p0_summary_wald_no_f)

save(df_p0_plot_data,
     file="~/Desktop/summer_2022/power_anova_lm_hetero.RData")
...
png("~/Desktop/summer_2022/lm_hetero_anova_power.png",
