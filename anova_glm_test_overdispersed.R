...
for (beta_2 in beta_2_seq){
  ...
  for (i in c(1:n_sim)){
    cat(i,"\n")
    y_sim <- MASS::rnegbin(n = nrow(mri), beta_0 + mri$weight*beta_1 +
                     mri$height*beta_2+mri$alcoh*beta_3, theta = 50)
  ...
  for (i in c(1:n_sim)){
    ...
    cat(i,"\n")
    y_sim <- MASS::rnegbin(n = nrow(mri_sub), beta_0 + mri_sub$weight*beta_1 +
                     mri_sub$height*beta_2+mri_sub$alcoh*beta_3, theta=50)
  ...
  #   p_val_collection_10_LRT[i] <- curr_p_LRT
  # }
...
save(df_p0_plot_data,
     file="~/Desktop/summer_2022/power_anova_glm_negbin.RData")
...
png("~/Desktop/summer_2022/glm_negbin_rate_anova_power.png",
